...
本文件提供本伺服器運行插件之指令、子命令、操作說明與權限節點對照，分類為玩家指令 (User Commands) 與管理員指令 (Admin/OP Commands)，供系統管理人員進行 LuckPerms 權限配置參考。
...
### 自訂 Skript 腳本說明與控制指令 (Custom Scripts)
以下為 `plugins/Skript/scripts/` 目錄下部署之客製化腳本所定義之特殊事件及指令：

#### 1. money.sk (自訂鈔票與支票系統)
* **玩家存入方式**：
  * 手持鈔票或支票，**滑鼠右鍵點擊**，可將面額存入個人遊戲幣餘額。
  * 持整疊同面額鈔票時，**按住 Shift 並點擊滑鼠右鍵**，可一次性存入整疊鈔票。
* **保護機制**：禁止在鐵砧中修改或取出 Lore 包含 `極光の幻想域專用鈔票` 的任何物品。
* **自訂指令**：
  * `/getmoney <金額> <玩家>`: 依據總金額，拆分成面額 1000/500/100/50/10/5/1 的實體鈔票給予指定玩家。(權限：`money.admin`)
  * `/getcheck <金額> <玩家>`: 給予玩家一張指定金額的自訂支票實體物。(權限：`money.admin`)

#### 2. signin.sk (補簽卷系統)
* **玩家使用方式**：手持「補簽卷」紙張，**滑鼠右鍵點擊**，可兌換 1 次每日簽到補簽次數。
* **自訂指令**：
  * `/signin_give <玩家> <數量>`: 發給指定玩家指定數量的補簽卷。(權限：`admin.misscard`)

#### 3. open_menu.sk (快捷選單系統)
* **玩家使用方式**：手持「極光の幻想鄉」選單時鐘，**滑鼠右鍵點擊**，將會開啟伺服器快捷選單。
* **自訂指令**：
  * `/getmenu`: 取得「極光の幻想鄉」選單時鐘道具（設有 60 秒獲取冷卻時間）。(權限：無)

#### 4. setthing.sk (物品快捷生成器)
* **自訂指令**：
  * `/setthing <物品ID> <名稱> <描述(使用|換行)> <數量>`: 管理員快速生成並取得自訂名稱與 Lore 的物品。(權限：`admin.setthing`)

#### 5. mysterious.sk (管理員旁觀切換)
* **自訂指令**：
  * `/st`: 管理員專用。快速在生存模式與旁觀者模式間進行切換，並自動賦予/收回 `/tp` 傳送權限。(權限：`sk.st.switch`)

#### 6. welcome.sk (新手登入初始化)
* **運作流程**：新玩家首次加入伺服器時，自動將權限群組設為 `Player`，發放選單時鐘及包含基本物資與裝備的「新手禮包」潛影盒，並傳送至大廳世界。

#### 7. noban.sk (特定玩家追蹤)
* **運作流程**：當玩家 `ez_cred_ray` 連入伺服器時，自動將其傳送至大廳世界世界坐標。
